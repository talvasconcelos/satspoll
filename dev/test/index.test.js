...
async function moduleWithHost() {
  ...
  const wallet = {
    ...
    createInvoicePublic: (request) => {
      invoiceRequests.push({ ...request, extra: { ...request.extra } });
      return {
        paymentHash: "hash",
  ...
  return { api, invoiceRequests, tables };
}
...
test("validates options and counts a paid event once", async () => {
  ...
  assert.equal(
    unwrap(api.createPoll(JSON.stringify({ ...base, options: ["Only"] }))).ok,
    false,
  );
  assert.equal(
    unwrap(api.createPoll(JSON.stringify({ ...base, options: ["A", "a"] }))).ok,
    false,
  );
  const poll = unwrap(
    api.createPoll(JSON.stringify({ ...base, options: ["Pizza", "Soup"] })),
  ).data;
  const optionId = poll.options[0].id;
  const event = {
    paymentHash: "paid_hash",
    sourceId: poll.id,
    amount: 21000,
    extra: { extra_satspoll: { optionId } },
  };
  assert.equal(
    unwrap(api.recordVote(JSON.stringify(event))).data.recorded,
    true,
  );
  tables.poll_options.find((row) => row.id === optionId).count = 0;
  assert.equal(
    unwrap(api.recordVote(JSON.stringify(event))).data.duplicate,
    true,
  );
  assert.equal(tables.votes.length, 1);
  const publicPoll = unwrap(
    api.getPublicPoll(JSON.stringify({ pollId: poll.id })),
  ).data;
...
test("invoice rejects closed, unknown, and foreign options without a vote", async () => {
  const { api, invoiceRequests, tables } = await moduleWithHost();
  ...
  const invoice = (payload) =>
    unwrap(api.createVoteInvoice(JSON.stringify(payload)));
  assert.equal(invoice({ pollId: first.id, optionId: "missing" }).ok, false);
  assert.equal(
    invoice({ pollId: first.id, optionId: second.options[0].id }).ok,
    false,
  );
  assert.equal(
    invoice({ pollId: first.id, optionId: first.options[0].id }).ok,
    true,
  );
  assert.deepEqual(invoiceRequests, [
    {
      ...
      currency: "sat",
      memo: "Vote: Poll",
      extra: { optionId: first.options[0].id },
    },
  ]);
...
test("updates only mutable fields and deletes unused polls", async () => {
  const { api, tables } = await moduleWithHost();
  const poll = unwrap(
    api.createPoll(
      JSON.stringify({
  ...
  assert.equal(updated.amountSats, 5);
  assert.equal(tables.polls[0].wallet_id, "wallet");
  assert.deepEqual(
  ...
  assert.equal(
    unwrap(api.deletePoll(JSON.stringify({ pollId: poll.id }))).data.deleted,
    true,
  );
  assert.equal(tables.polls.length, 0);
  assert.equal(tables.poll_options.length, 0);
});
